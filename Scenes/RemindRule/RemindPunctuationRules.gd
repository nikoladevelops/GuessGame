...
func _ready():
	$RichTextLabel.text="Избери пунктуационно правило:";

func _loadDataFromDb():
	""" Load data from db """
	db.query("SELECT * FROM PunctuationRules");
...
func _on_RuleList_item_selected(index):
	...
	var currentGrammarRuleDetails = db.query("SELECT * FROM PunctuationRules WHERE Name="+"'"+currentSelectedGrammarRuleName+"'");
